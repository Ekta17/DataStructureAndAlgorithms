...
public class PrefixSum {
  ...
  /**
   * Given an array, and multiple queries to find the sum of a range of numbers in the array from
   * low to high (inclusive). Write a code to efficiently find the sum in time complexity O(1)
   *
  ...
  public int getSum(int[] prefixSum, int low, int high) {
    if (low == 0) return prefixSum[high];
    else return prefixSum[high] - prefixSum[low - 1];
  }
